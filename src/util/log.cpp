...
bool g_is_log_enable = false; //日志开关，默认关闭，由用户通过命令行参数控制
...
Logger::Logger() : log_level_(LogLevel::DEBUG), log_directory_("./log/") {
  CreateLogDirectory();
  InitLogFile();
}
...
void Logger::CreateLogDirectory() {
  ...
  if (stat(log_directory_.c_str(), &st) != 0) {
    if (mkdir(log_directory_.c_str(), 0755) != 0) {
      std::cerr << "Failed to create log directory: " << log_directory_
                << std::endl;
    }
  }
}
